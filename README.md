...
- **Pre-submission failures retry automatically; post-submission ones don't (2026-08-22).** A failure before the confirm button is ever clicked (page load, market card lookup, odds read — nothing submitted yet) is safe to retry, since there's no risk of a duplicate bet — `processBet.ts` reverts the row to `approved` and the next cycle tries again, up to 10 attempts (`PreSubmitError` in `betpawa/types.ts`). A failure from the confirm click onward stays genuinely ambiguous (did it go through or not?) and is never auto-retried — a blind retry there risks placing the same bet twice, a worse outcome than missing it. Both cases notify Telegram; only the second needs you to actually look at anything.
...
Project Pi computes a Poisson model + a heuristic "Pressure Model" + situational signals (relegation desperation, draw-trap flags) from football-data.org fixtures/stats, then asks Claude to pick a bet. Its `analyze-matches` function (in the separate `project-pi` repo, not here) writes to its own `recommendations` table **and**, in the same run, bridges into this project's `recommended_bets` — mapping `bet_type`/`pick` onto BetPawa's actual market vocabulary: `1X2`, `Double Chance`, `BTTS`, `Over/Under 2.5`, `Draw No Bet`. Over/Under 2.5 was briefly retired (2026-08-16) as too risky, then reinstated (2026-08-17) by operator call. Draw No Bet was excluded from the very start of this project ("no confirmed BetPawa market for it yet") until 2026-08-22, when live recon confirmed BetPawa does offer it ("Draw No Bet | Full Time", labels "1"/"2") — it's the one market with a genuine void/push outcome (a draw refunds the stake; see `determineOutcomeForBet` in `settle-results`), and `model_probability` for it is `P(win | not a draw)` rather than the raw Poisson win probability, since Kelly needs the probability conditional on the bet actually resolving. Persists `model_probability` and `pi_match_id` (links back to Project Pi's own `matches` row, used by settlement — see below), and leaving `bookmaker_event_url` null (a Deno edge function can't run Playwright). `recommended_stake` (a flat confidence-tier number) is written too but is purely an **advisory display value** for the human approval step — it never governs real money; see "Money management" below for what actually decides the stake.
...
- **Settlement** (win/loss/void — lives in the separate `project-pi` repo's `settle-results` function, since that's where settlement runs).

Uses a **dedicated bot** ("AI Bet UG Alerts"), not project-pi's existing Telegram setup. That existing setup turned out to be a shared-project trap: `project-pi`'s Supabase project (`vkaprrhkmbbhagcidaka`) is also the backend for an unrelated app, **`project-lydia`** (SMS finance tracking + meeting reminders, via `telegram-webhook`/`sms-ingest`/`lydia-meeting`), which already owned the generic secret names `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` on that project for its own, different bot. Reusing those names for this feature's first version silently redirected/broke Lydia's bot (2026-08-16) until caught and fixed. `settle-results` now reads `AI_BET_TELEGRAM_BOT_TOKEN`/`AI_BET_TELEGRAM_CHAT_ID` instead — deliberately distinct names, own bot, no collision.

**If you ever add a Supabase secret on the `project-pi` project**: check `supabase secrets list --project-ref vkaprrhkmbbhagcidaka` first and never reuse an existing generic-sounding name without confirming what else on that project depends on it.

Set `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` in **this repo's own** `.env` locally and as **this repo's own** GitHub Actions secrets (same pattern as the other four — these are ai-bet-ug's own Node/Actions secrets, a completely separate store from project-pi's Supabase secrets, so no collision risk here) to enable placement/failure/drawdown notifications. Leave both unset to disable — nothing else changes, notifications just silently no-op.
